...
export default (class {
  ...
  constructor(namespace = "exlg", redis = createClient()) {
    ...
    const respond = (
      ...
      res
        .status(status)
        .json({ status, ...(status < 400 ? { data } : { error: data }) });
    };

    const tokenReuired = async (
      req: Request,
      res: Response,
      next: NextFunction
    ) => {
      if (
        req.body.uid &&
        req.body.token &&
        (await redis.get(`${namespace}:token:${req.body.token}`)) ===
        ...
        if (!(await redis.get(`${namespace}:${req.body.uid}:blacklisted`))) {
          ...
          respond(
            res,
            403,
            "You've been blacklisted! Please contact the admin."
          );
    ...
    const activationRequired = async (
      ...
      (req.body.activation &&
        ...
        ? next()
        : respond(res, 402, "Invalid activation");
    ...
    app.get("/token/verify/:paste", async (req, res) => {
      ...
      const response = await axios.get<
        DataResponse<{ paste: Paste; errorMessage: string }>
      ...
      if (response.data.code >= 400) {
        ...
        if ((await redis.get(`${namespace}:token:${data.data}`)) === "0") {
          await redis.set(`${namespace}:token:${data.data}`, data.user.uid, {
            EX: 60 * 60 * 24 * 3,
          });
          respond(res, 200, {
    ...
    app.post("/token/ttl", tokenReuired, async (req, res) => {
      respond(
        res,
        200,
        await redis.ttl(`${namespace}:token:${req.body.token}`)
      );
    });
    ...
    app.post("/badge/mget", tokenReuired, dataRequired, async (req, res) => {
      ...
      respond(res, 200, Object.fromEntries(uids.map((k, i) => [k, badges[i]])));
    });
    ...
    app.post(
      ...
      (req, res, next) => {
        ...
        if (error.length) {
          ...
          req.body.data.text = req.body.data.text
            .trim()
            .replaceAll("&", "&amp;")
            .replaceAll("<", "&lt;")
            .replaceAll(">", "&gt;");
